...
async function fetchFacebookData(endpoint) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`⚠️ Error fetching FB data for ${endpoint}:`, errText);
      return null;
    }
    return await response.json();
  } catch (error) {
    console.error(`⚠️ Network error fetching FB data for ${endpoint}:`, error.message);
    return null;
  }
}
...
async function getPageMetrics() {
  if (!PAGE_ID || !ACCESS_TOKEN) {
    throw new Error('Missing PAGE_ID or PAGE_ACCESS_TOKEN environment variables.');
  }

  console.log('📊 [1/5] Obteniendo información general de la página "Para eso trabajo"...');
  ...
  return {
    pageName: pageInfo ? pageInfo.name : 'Para eso trabajo',
...
📊 *REPORTE DE RENDIMIENTO SEMANAL - PARA ESO TRABAJO*
...
4. 🧠 *Lectura Estratégica e Insights*:
   - Qué temas, ofertas de productos o tipos de publicaciones de afiliación muestran mejor recepción y engagement.

5. 🛠️ *Estrategia y Acciones a Aplicar*:
   - Tres acciones ultra-concretas que el bot aplicará en la selección de ofertas, ganchos de texto y tipos de posts para la próxima semana para optimizar ventas y conversiones.
